...
class GitGutterCommand(sublime_plugin.WindowCommand):
        ...
        if ViewCollection.untracked(self.view):
            ...
            if(ViewCollection.show_status(self.view) != "none"):
                if(ViewCollection.show_status(self.view) == 'all'):
                    ...
                    compare = ViewCollection.get_compare(self.view)
                elif(ViewCollection.show_status(self.view) == 'more'):
                    branch = ViewCollection.current_branch(
                        self.view).decode("utf-8").strip()
                    compare = ""
                else:
    ...
    def update_status(self, inserted, modified, deleted, compare, branch):
        ...
        set_status_if(inserted > 0, "inserted", "%d+" % inserted)
        set_status_if(modified > 0, "modified", "%d≠" % modified)
        set_status_if(deleted > 0, "deleted", "%d-" % deleted)
        set_status_if(compare, "comparison", "Comparing: %s" % compare)
        set_status_if(branch, "branch", "Branch: %s" % branch)
